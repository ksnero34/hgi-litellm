  ...
  if (isViewOnly) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center gap-2 p-8 text-center">
        <h1 className="text-2xl font-semibold">{t("playgroundAgents.chat.accessDenied")}</h1>
        <p className="text-muted-foreground">{t("playgroundAgents.chat.accessDeniedHelp")}</p>
      </div>
    );
  }
      ...
      <Tabs defaultValue="chat" className="flex min-h-0 min-w-0 flex-1 flex-col gap-0 overflow-hidden">
        <TabsList variant="line" className="w-full shrink-0 justify-start overflow-x-auto pb-1">
          <TabsTrigger value="chat" className="flex-none">
            {t("interactionExtra.playground.chat")}
          </TabsTrigger>
          <TabsTrigger value="compare" className="flex-none">
            {t("interactionExtra.playground.compare")}
          </TabsTrigger>
          <TabsTrigger value="compliance" className="flex-none">
            {t("interactionExtra.playground.compliance")}
          </TabsTrigger>
          <TabsTrigger value="agent-builder" className="flex-none">
            {t("interactionExtra.playground.agentBuilder")}
          </TabsTrigger>
        </TabsList>
        <TabsContent
          value="chat"
