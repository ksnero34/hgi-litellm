  ...
  return (
    <div className="flex flex-col gap-6 min-w-0 w-full p-4">
      {conversationBlocks.map((block, blockIndex) => {
        ...
        const displayModel = assistantMessage?.model || t("interactionExtra.playground.assistant");
        return (
          <div key={blockIndex} className="space-y-4">
            {block.user && (
              <div className="space-y-2 min-w-0">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-sm font-semibold text-gray-700">{t("interactionExtra.playground.you")}</div>
                </div>
            ...
            {assistantMessage ? (
              ...
              <div className="flex items-center gap-2 text-sm text-gray-500">
                ...
                <span>{t("interactionExtra.playground.generating")}</span>
              </div>
            ) : (
              <div className="text-sm text-gray-500">{t("interactionExtra.playground.waiting")}</div>
            )}
          </div>
        );
      })}
