...
function AgentRowActions({ agent, onDeleteClick, t }: AgentRowActionsProps) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        aria-label={t("identityAdmin.agents.openActions")}
...
export const getAgentsTableColumns = ({
...
}: AgentsTableColumnsDeps): ColumnDef<Agent>[] => [
  {
    ...
    meta: { title: t("identityAdmin.agents.name") },
    header: ({ column }) => <DataTableSortHeader column={column} title={t("identityAdmin.agents.name")} />,
    size: 200,
    enableSorting: true,
    cell: ({ row }) => {
  ...
  },
  {
    ...
    meta: { title: t("identityAdmin.agents.id") },
    header: ({ column }) => <DataTableSortHeader column={column} title={t("identityAdmin.agents.id")} />,
    size: 200,
    enableSorting: true,
    cell: ({ row }) => (
      <IdentityCell
  ...
  },
  {
    id: "spend",
    accessorKey: "spend",
    meta: { title: t("identityAdmin.agents.spend") },
    header: ({ column }) => <DataTableSortHeader column={column} title={t("identityAdmin.agents.spend")} />,
    size: 130,
    enableSorting: true,
  ...
  {
    id: "model",
    meta: { title: t("identityAdmin.agents.model") },
    header: t("identityAdmin.agents.model"),
    size: 170,
    enableSorting: false,
    cell: ({ row }) => {
      const model = row.original.litellm_params?.model;
      if (!model) {
        return <span className="text-muted-foreground">{t("identityAdmin.agents.notAvailable")}</span>;
      }
      return (
  ...
  },
  {
    ...
    meta: { title: t("identityAdmin.agents.created") },
    header: ({ column }) => <DataTableSortHeader column={column} title={t("identityAdmin.agents.created")} />,
    size: 150,
    enableSorting: true,
  ...
  {
    id: "status",
    meta: { title: t("identityAdmin.users.status") },
    ...
    cell: ({ row }) => {
      ...
      return hasKeys ? (
        <StatusBadge tone="success" label={t("identityAdmin.users.active")} />
      ) : (
        <StatusBadge tone="warning" label={t("identityAdmin.agents.needsSetup")} />
      );
    },
  },
  ...(isAdmin
