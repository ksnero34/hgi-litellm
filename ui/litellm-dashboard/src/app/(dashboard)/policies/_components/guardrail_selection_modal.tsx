    ...
    <Modal
      title={
        <div>
          ...
          <p className="text-sm text-gray-500 font-normal mt-1">{t("policies.guardrailSelection.reviewTitle")}</p>
        </div>
      }
      open={visible}
      onCancel={onCancel}
      ...
      <div className="py-4">
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {guardrail.alreadyExists && (
                      <Tag color="green" className="text-xs">
                        {t("policies.guardrailSelection.alreadyExists")}
                      </Tag>
                    )}
                  </div>
        ...
        <div className="text-sm text-gray-600">
          {selectedCount > 0 ? (
            <p>
              <span className="font-medium text-gray-900">
                {t("policies.guardrailSelection.selectedSummary", { count: selectedCount })}
              </span>
            </p>
          ) : existingCount > 0 ? (
            <p className="text-green-600">{t("policies.guardrailSelection.allExisting")}</p>
          ) : (
            <p className="text-orange-600">{t("policies.guardrailSelection.selectPrompt")}</p>
          )}
        </div>
      </div>
    </Modal>
