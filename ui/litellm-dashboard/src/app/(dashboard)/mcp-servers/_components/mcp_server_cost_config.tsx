      ...
      <Card className="p-6">
        <div className="space-y-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-lg font-medium">{t("toolsModels.mcp.costConfiguration")}</h3>
            <Tooltip>
              <TooltipTrigger
                render={<Info className="size-4 text-muted-foreground" aria-label="About cost configuration" />}
              />
              <TooltipContent>{t("toolsModels.mcp.costConfigurationHelp")}</TooltipContent>
            </Tooltip>
          </div>
          ...
          <div className="space-y-4">
            <div>
              <label className="mb-2 block text-sm font-medium">
                {t("toolsModels.mcp.defaultCostPerQueryWithCurrency")}
                <Tooltip>
                  <TooltipTrigger
                    render={
                  ...
                  />
                  <TooltipContent>{t("toolsModels.mcp.defaultCostPerQueryHelp")}</TooltipContent>
                </Tooltip>
              </label>
              <CostInput
              ...
              />
              <p className="mt-1 block text-sm text-muted-foreground">{t("toolsModels.mcp.defaultCostForAllTools")}</p>
            </div>

            {tools.length > 0 && (
              <div className="space-y-4">
                <label className="block text-sm font-medium">
                  {t("toolsModels.mcp.toolSpecificCostsWithCurrency")}
                  <Tooltip>
                    <TooltipTrigger
                      render={
                    ...
                    />
                    <TooltipContent>{t("toolsModels.mcp.toolSpecificCostsHelp")}</TooltipContent>
                  </Tooltip>
                </label>
                  ...
                  <CollapsibleTrigger
                    render={
                      <button type="button" className="flex w-full items-center gap-2 p-3 text-left">
                        ...
                        <span className="font-medium">{t("toolsModels.mcp.availableTools")}</span>
                        ...
                        <div key={index} className="flex items-center justify-between rounded-lg bg-muted p-3">
                          ...
                          <div className="ml-4">
                            <CostInput
                              ...
                              placeholder={t("toolsModels.mcp.useDefaultCost")}
                              disabled={disabled}
                              className="w-40"
          ...
          {(value.default_cost_per_query ||
            ...
            <div className="mt-6 rounded-lg border border-border bg-muted p-4">
              <p className="text-sm font-medium">{t("toolsModels.mcp.costSummary")}</p>
              <div className="mt-2 space-y-1">
                {value.default_cost_per_query && (
                  <p className="text-sm text-muted-foreground">
                    {t("toolsModels.mcp.defaultCostSummary", { cost: value.default_cost_per_query.toFixed(4) })}
                  </p>
                )}
                {value.tool_name_to_cost_per_query &&
                    ...
                    ([toolName, cost]) =>
                      ...
                      cost !== undefined && (
                        <p key={toolName} className="text-sm text-muted-foreground">
                          {t("toolsModels.mcp.toolCostSummary", { toolName, cost: cost.toFixed(4) })}
                        </p>
                      ),
                  )}
              </div>
