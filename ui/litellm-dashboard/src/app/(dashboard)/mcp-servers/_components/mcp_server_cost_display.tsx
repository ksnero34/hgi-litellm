      ...
      <div className="mt-6 border-t border-border pt-6">
        <div className="space-y-4">
          <div className="rounded-lg border border-border bg-muted p-4">
            <p className="text-sm text-muted-foreground">{t("toolsModels.mcp.noCostConfiguration")}</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="space-y-4">
        {hasDefaultCost &&
          ...
          costConfig?.default_cost_per_query !== null && (
            <div>
              <p className="text-sm font-medium">{t("toolsModels.mcp.defaultCostPerQuery")}</p>
        ...
        {hasToolCosts && costConfig?.tool_name_to_cost_per_query && (
          <div>
            <p className="text-sm font-medium">{t("toolsModels.mcp.toolSpecificCosts")}</p>
                  ...
                  cost !== undefined && (
                    <div key={toolName} className="flex items-center justify-between rounded-lg bg-muted p-3">
                      ...
                      <p className="font-mono text-sm">
                        {t("toolsModels.mcp.costPerQuery", { cost: cost.toFixed(4) })}
                      </p>
                    </div>
                  ),
              )}
            ...
            {hasDefaultCost &&
              ...
              costConfig?.default_cost_per_query !== null && (
                <p className="text-sm text-muted-foreground">
                  {t("toolsModels.mcp.defaultCostSummary", { cost: costConfig.default_cost_per_query.toFixed(4) })}
                </p>
              )}
            {hasToolCosts && costConfig?.tool_name_to_cost_per_query && (
              <p className="text-sm text-muted-foreground">
                {t("toolsModels.mcp.customPricedToolsSummary", {
