...
function EmptyState() {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-base font-semibold text-foreground">{t("modelManagement.noModels")}</div>
      <div className="max-w-80 text-sm text-muted-foreground">{t("modelManagement.noModelsMatchFilters")}</div>
    </div>
  );
}
...
export function AllModelsTable({
  ...
  const filterLabels = useMemo(
    () => ({
      [MODEL_NAME_COLUMN_ID]: t("modelManagement.publicModelName"),
      [ACCESS_GROUPS_COLUMN_ID]: t("modelManagement.accessGroup"),
    }),
    [t],
  );
  const viewModeLabels = useMemo(
    () => ({
      current_team: t("modelManagement.currentTeamModels"),
      all: t("modelManagement.allAvailableModels"),
    }),
    [t],
  );
  ...
  const modelGroupOptions = useMemo(
    () => [
      { label: t("modelManagement.allModels"), value: ALL_MODEL_GROUPS_VALUE },
      { label: t("modelManagement.wildcardModels"), value: WILDCARD_MODEL_GROUP_VALUE },
  ...
  const accessGroupOptions = useMemo(
    () => [
      { label: t("modelManagement.allAccessGroups"), value: ALL_MODEL_GROUPS_VALUE },
  ...
  const formatFilterValue = (columnId: string, value: unknown): string => {
    ...
    if (columnId === MODEL_NAME_COLUMN_ID && raw === WILDCARD_MODEL_GROUP_VALUE) {
      return t("modelManagement.wildcardModels");
    }
    return raw;
  };
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      loadingMessage={t("modelManagement.loadingModels")}
      ...
      toolbar={(table) => (
        <>
          <DataTableToolbar
            ...
            searchPlaceholder={t("modelManagement.searchModelNames")}
          ...
          >
            <Select value={selectedTeamValue} onValueChange={(value) => onTeamChange(String(value))}>
              <SelectTrigger
                size="sm"
                aria-label={t("modelManagement.selectTeam")}
            ...
            <Select value={viewMode} onValueChange={(value) => onViewModeChange(value as ModelViewMode)}>
              <SelectTrigger
                size="sm"
                aria-label={t("modelManagement.view")}
                data-testid="models-view-select"
                className="gap-2"
              >
                <span className="text-muted-foreground">{t("modelManagement.view")}</span>
            ...
            <Button
              variant="outline"
              size="icon-sm"
              aria-label={t("modelManagement.modelSettings")}
          ...
          <DataTableFilterDrawer
            ...
            title={t("modelManagement.filters")}
            description={t("modelManagement.filtersDescription")}
            resetLabel={t("modelManagement.resetFilters")}
            onReset={onResetFilters}
          >
            {({ get, set }) => (
              <>
                <DataTableFilterField label={t("modelManagement.publicModelName")}>
                  <SearchSelect
                    ...
                    emptyText={t("modelManagement.noModels")}
                  />
                </DataTableFilterField>
                <DataTableFilterField label={t("modelManagement.accessGroup")}>
                  <SearchSelect
                    ...
                    emptyText={t("modelManagement.noModelAccessGroupsFound")}
                  />
                </DataTableFilterField>
              </>
            )}
