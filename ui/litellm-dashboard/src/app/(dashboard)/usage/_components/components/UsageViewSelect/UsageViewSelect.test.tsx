...
describe("UsageViewSelect", () => {
  ...
  it("should hide Tag Usage for non-admin users even when tag capability is present", async () => {
    const user = userEvent.setup();
    const { container } = render(
      <UsageViewSelect value="my-usage" onChange={mockOnChange} userRole="Internal User" canViewTagUsage={true} />,
    );
  ...
  it("should keep Team Usage available to an internal user", async () => {
    const user = userEvent.setup();
    const { container } = render(
      <UsageViewSelect value="my-usage" onChange={mockOnChange} userRole="Internal User" canViewTagUsage={true} />,
    );
    ...
    await openMenu(user);
    expect(offers(container, "Team Usage")).toBe(true);
  });
});
