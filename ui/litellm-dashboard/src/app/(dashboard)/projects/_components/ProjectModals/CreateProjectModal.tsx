  ...
  const handleSubmit = async () => {
    try {
      ...
      createMutation.mutate(params, {
        onSuccess: () => {
          MessageManager.success(t("access.projects.modal.createSuccess"));
          form.resetFields();
          onClose();
        },
        onError: (error) => {
          MessageManager.error(error.message || t("access.projects.modal.createError"));
        },
      });
    } catch (error) {
    ...
    <Modal
      title={
        <Typography.Text strong style={{ fontSize: 18 }}>
          {t("access.projects.modal.createTitle")}
        </Typography.Text>
      }
      open={isOpen}
