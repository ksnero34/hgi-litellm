...
const PerUserUsage: React.FC<PerUserUsageProps> = ({ accessToken, selectedTags, formatAbbreviatedNumber }) => {
  ...
  const columns: ColumnDef<PerUserMetrics>[] = [
    {
      header: t("observabilityExtra.usage.userId"),
      accessorKey: "user_id",
      cell: ({ row }) => <span className="font-medium">{row.original.user_id}</span>,
    },
    {
      header: t("observabilityExtra.usage.userEmail"),
      accessorKey: "user_email",
      cell: ({ row }) => row.original.user_email || t("observabilityExtra.usage.notAvailable"),
    },
    {
      header: t("observabilityExtra.usage.userAgent"),
      accessorKey: "user_agent",
      cell: ({ row }) => row.original.user_agent || t("observabilityExtra.usage.unknown"),
    },
    {
      header: t("observabilityExtra.usage.successGenerations"),
      accessorKey: "successful_requests",
      meta: { numeric: true },
      cell: ({ row }) => formatAbbreviatedNumber(row.original.successful_requests),
    },
    {
      header: t("observabilityExtra.usage.totalTokens"),
      accessorKey: "total_tokens",
      meta: { numeric: true },
      cell: ({ row }) => formatAbbreviatedNumber(row.original.total_tokens),
    },
    {
      header: t("observabilityExtra.usage.failedRequests"),
      accessorKey: "failed_requests",
      meta: { numeric: true },
      cell: ({ row }) => formatAbbreviatedNumber(row.original.failed_requests),
    },
    {
      header: t("observabilityExtra.usage.totalCost"),
      accessorKey: "spend",
      meta: { numeric: true },
  ...
  return (
    <div className="mb-6">
      <h3 className="text-lg font-medium text-foreground">{t("observabilityExtra.usage.perUser")}</h3>
      <p className="text-sm text-muted-foreground">{t("observabilityExtra.usage.perUserDescription")}</p>

      <Tabs defaultValue="details">
        <TabsList className="mb-6">
          <TabsTrigger value="details" className="flex-none px-3">
            {t("observabilityExtra.usage.userDetails")}
          </TabsTrigger>
          <TabsTrigger value="distribution" className="flex-none px-3">
            {t("observabilityExtra.usage.distribution")}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="details" keepMounted>
          <DataTable
            ...
            noDataMessage={t("observabilityExtra.usage.noPerUserData")}
            size="compact"
          />
          ...
          {perUserData.results.length > 10 && (
            <div className="mt-4 flex justify-between items-center">
              <p className="text-sm text-gray-500">
                {t("observabilityExtra.usage.showingResults", { shown: 10, total: perUserData.total_count })}
              </p>
              <div className="flex gap-2">
                <Button size="sm" variant="secondary" onClick={handlePrevPage} disabled={currentPage === 1}>
                  {t("observabilityExtra.usage.previous")}
                </Button>
                <Button
                  size="sm"
                  variant="secondary"
                ...
                >
                  {t("observabilityExtra.usage.next")}
                </Button>
              </div>
            </div>
          )}
        ...
        <TabsContent value="distribution" keepMounted>
          <div className="mb-4">
            <h4 className="text-lg font-medium text-foreground">{t("observabilityExtra.usage.userDistribution")}</h4>
            <p className="text-sm text-muted-foreground">{t("observabilityExtra.usage.userDistributionDescription")}</p>
          </div>
          ...
          <BarChart
            ...
            valueFormatter={(value: number) => t("observabilityExtra.usage.usersCount", { count: value })}
            yAxisWidth={80}
            showLegend={true}
            stack={true}
          />
