...
function EmptyState() {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{t("modelManagement.noCredentialsConfigured")}</div>
      <div className="text-sm text-muted-foreground">{t("modelManagement.addCredentialToConnectProvider")}</div>
    </div>
  );
}
...
const CredentialsTable: React.FC<CredentialsTableProps> = ({
  ...
  const columns = useMemo(
    () => getCredentialsTableColumns({ canModifyCredentials, t, onEdit, onDelete }),
    [canModifyCredentials, t, onEdit, onDelete],
  );
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      loadingMessage={t("modelManagement.loadingCredentials")}
