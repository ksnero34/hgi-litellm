...
const Fallbacks: React.FC<FallbacksProps> = ({ accessToken, userRole, userID }) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("settingsExtra.fallbacks.updateFailed", { error: String(error) }));
    } finally {
      setIsDeleting(false);
    ...
    <TooltipProvider>
      ...
      {!hasFallbacks ? (
        <div className="rounded-lg border border-gray-200 bg-gray-50 px-4 py-6 text-center">
          <span className="text-muted-foreground">{t("settingsExtra.fallbacks.noneConfigured")}</span>
        </div>
      ) : (
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>{t("settingsExtra.fallbacks.modelName")}</TableHead>
              <TableHead>{t("settingsExtra.fallbacks.fallbacks")}</TableHead>
              <TableHead>{t("settingsExtra.common.actions")}</TableHead>
            </TableRow>
          </TableHeader>
                    ...
                    {canModify && (
                      <>
                        <Tooltip>
                          ...
                          <TooltipContent>{t("settingsExtra.fallbacks.test")}</TooltipContent>
                        </Tooltip>
                        <Tooltip>
                          <TooltipTrigger
                            render={
                          ...
                          <TooltipContent>{t("settingsExtra.fallbacks.edit")}</TooltipContent>
                        </Tooltip>
                        <Tooltip>
                          <TooltipTrigger
                            render={
                          ...
                          <TooltipContent>{t("settingsExtra.fallbacks.delete")}</TooltipContent>
                        </Tooltip>
                      </>
                    )}
                  </TableCell>
