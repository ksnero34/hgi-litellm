  ...
  const handleFormSubmit = (formValues: LoggingSettingsFormValues) => {
    ...
    void clearStoredFieldsInSequence(fieldsToClear).then((failed) => {
      if (failed.length > 0) {
        ...
        NotificationsManager.fromBackend(t("settingsExtra.logging.clearFailed", { fields: failed.join(", ") }));
        return;
      }
      submitUpdate();
    });
        ...
        {isLoadingConfig ? (
          ...
          <Form form={form} layout="vertical" onFinish={handleFormSubmit} initialValues={initialValues}>
            <Form.Item
              ...
              tooltip={describeField(STORE_PROMPTS_FIELD_NAME, t("settingsExtra.logging.storePromptsDescription"))}
            ...
            {OPTIONAL_FIELDS.map((field) => (
              <Form.Item
                key={field.name}
                label={t(`settingsExtra.logging.optional.${field.name}.label`, { defaultValue: field.label })}
                name={field.name}
                tooltip={describeField(
                  field.name,
                  t(`settingsExtra.logging.optional.${field.name}.description`, {
                    defaultValue: field.fallbackTooltip,
                  }),
                )}
              >
                {field.kind === "duration" ? (
                  <Input
                    placeholder={t(`settingsExtra.logging.optional.${field.name}.placeholder`, {
