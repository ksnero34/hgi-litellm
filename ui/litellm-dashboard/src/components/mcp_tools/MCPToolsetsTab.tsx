...
function toolsetColumns(
  ...
  context: { serverPrefixById: Map<string, string>; t: TFunction },
): ColumnDef<MCPToolset>[] {
  const { serverPrefixById, t } = context;
  const proxyBaseUrl = getProxyBaseUrl();
  return [
    {
      header: t("toolsModels.mcp.toolsets.columns.id"),
      accessorKey: "toolset_id",
      cell: ({ row }) => <IdCell value={row.original.toolset_id} />,
    },
    {
      header: t("toolsModels.mcp.common.name"),
        ...
        return (
          <div className="flex flex-col gap-0.5">
            ...
            <button
              ...
              title={t("toolsModels.mcp.toolsets.copyEndpoint")}
            >
              {url}
            </button>
          </div>
    ...
    {
      header: t("toolsModels.mcp.common.description"),
      accessorKey: "description",
      cell: ({ row }) => <span className="text-sm text-gray-500">{row.original.description || "—"}</span>,
    },
    {
      header: t("toolsModels.mcp.toolsets.columns.tools"),
      accessorKey: "tools",
      cell: ({ row }) => {
          ...
          <div className="flex flex-wrap gap-1 max-w-xs">
            ...
            {tools.length > 4 && (
              <span className="text-xs text-gray-400 self-center">
                {t("toolsModels.mcp.toolsets.moreTools", { count: tools.length - 4 })}
              </span>
            )}
          </div>
        );
    ...
    {
      header: t("toolsModels.mcp.toolsets.columns.created"),
...
export function MCPToolsetsTab({ accessToken, userRole }: MCPToolsetsTabProps) {
  ...
  const columns = toolsetColumns(isAdmin, setEditToolset, setDeleteId, { serverPrefixById, t });
