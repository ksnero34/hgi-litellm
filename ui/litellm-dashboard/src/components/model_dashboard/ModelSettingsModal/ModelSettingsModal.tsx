...
const ModelSettingsModal: React.FC<ModelSettingsModalProps> = ({ isVisible, onCancel, onSuccess }) => {
  ...
  const handleFormSubmit = async (formValues: StoreModelInDBParams) => {
    try {
      await mutateAsync(formValues, {
        onSuccess: () => {
          NotificationsManager.success(t("modelManagement.storeUpdated"));
          refetch();
          onSuccess?.();
        },
        onError: (error) => {
          NotificationsManager.fromBackend(t("modelManagement.storeSaveFailed", { message: parseErrorMessage(error) }));
        },
      });
    } catch (error) {
      NotificationsManager.fromBackend(t("modelManagement.storeSaveFailed", { message: parseErrorMessage(error) }));
  ...
  return (
    <Modal
      title={<Typography.Title level={5}>{t("modelManagement.modelSettings")}</Typography.Title>}
      open={isVisible}
      footer={
        <Space>
          ...
          <Button type="primary" loading={isPending} disabled={isLoadingConfig} onClick={() => form.submit()}>
            {isPending ? t("modelManagement.saving") : t("modelManagement.saveSettings")}
          </Button>
        </Space>
      }
