...
async def new_team(
    ...
    try:
        ...
        if is_audit_logging_enabled():
            _updated_values = safe_dumps(complete_team_data.model_dump(exclude_none=True))
...
async def _create_team_update_audit_log(
    ...
    _before_value = safe_dumps(existing_team_row.model_dump(exclude_none=True))
    _after_value: Final[str] = safe_dumps(updated_kv)
...
def _member_update_audit_value(members: Sequence[Member], data: TeamMemberUpdateRequest) -> str:
    return safe_dumps(
        {
            "members_with_roles": tuple(member.model_dump() for member in members),
...
async def _create_team_member_add_audit_logs(
    ...
    membership_entry: Final = create_object_audit_log(
        ...
        user_api_key_dict=user_api_key_dict,
        litellm_proxy_admin_name=litellm_proxy_admin_name,
        table_name=LitellmTableNames.TEAM_MEMBERSHIP_TABLE_NAME,
...
async def _create_team_membership_audit_log(
    ...
    await create_object_audit_log(
        ...
        table_name=LitellmTableNames.TEAM_MEMBERSHIP_TABLE_NAME,
        before_value=_members_audit_value(before_members),
        after_value=after_value or _members_audit_value(after_members),
    )
...
async def team_member_delete(
    ...
    members_before_delete = tuple(existing_team_row.members_with_roles)
    ...
    await _create_team_membership_audit_log(
        team_id=data.team_id,
        before_members=members_before_delete,
        after_members=tuple(existing_team_row.members_with_roles),
        user_api_key_dict=user_api_key_dict,
        action="deleted",
    )
...
async def team_member_update(
    ...
    members_before_update = tuple(team_table.members_with_roles)
    ...
    await _create_team_membership_audit_log(
        team_id=data.team_id,
        before_members=members_before_update,
        after_members=tuple(team_table.members_with_roles),
        user_api_key_dict=user_api_key_dict,
        action="updated",
        after_value=_member_update_audit_value(team_table.members_with_roles, data),
    )
...
async def delete_team(
    ...
    await asyncio.gather(
        *(
            create_object_audit_log(
                object_id=team_row.team_id,
                action="deleted",
                litellm_changed_by=litellm_changed_by,
                user_api_key_dict=user_api_key_dict,
                litellm_proxy_admin_name=litellm_proxy_admin_name,
                table_name=LitellmTableNames.TEAM_TABLE_NAME,
                before_value=safe_dumps(team_row.model_dump(exclude_none=True)),
                after_value=None,
            )
            for team_row in team_rows
        )
...
async def _resolve_team_daily_activity_scope(
    *,
    ...
    final_api_key_filter: str | list[str] | None = list(oidc_key_hashes) if oidc_key_hashes is not None else api_key
