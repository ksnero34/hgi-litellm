...
from typing import Any, Final, Literal, NamedTuple, Optional, Protocol, TypeVar, cast
...
async def _insert_deprecated_key(
    ...
    """
    grace_period_value: Final = grace_period or LITELLM_KEY_ROTATION_GRACE_PERIOD
    ...
    try:
        now: Final = datetime.now(timezone.utc)
        revoke_at: Final = now + timedelta(seconds=grace_seconds)
        ancestor_rows: Final = await deprecated_table.find_many(
            where={
                "active_token_id": old_token_hash,
                "revoke_at": {"gt": now},
            }
        )
        ancestor_hashes: Final = [row.token for row in ancestor_rows if row.token]
...
async def _execute_virtual_key_regeneration(
    *,
    ...
    if litellm_changed_by != LITELLM_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME:
...
def _build_key_filter_conditions(
    ...
    global_filters: Final[tuple[dict[str, object], ...]] = (
        ...
        *(({"OR": [{"blocked": False}, {"blocked": None}]},) if active_only else ()),
        *((_build_expires_where_clause("active", datetime.now(timezone.utc)),) if active_only else ()),
    )
    ...
    if record is not None:
        await create_audit_log_for_update(
            request_data=LiteLLM_AuditLogs(
                ...
                action="blocked",
                updated_values=json.dumps({"blocked": True}),
    ...
    if record is not None:
        await create_audit_log_for_update(
            request_data=LiteLLM_AuditLogs(
                ...
                object_id=hashed_token,
                action="unblocked",
                updated_values=json.dumps({"blocked": False}),
                before_value=existing_record.model_dump_json(exclude_none=True),
            ),
            mandatory=True,
        )
