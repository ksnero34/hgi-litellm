...
def generic_response_convertor(
    ...
    sso_team_mapping_configured: Final = team_mappings is not None and team_mappings.team_ids_jwt_field is not None
    ...
    if sso_team_mapping_configured and team_mappings is not None and team_mappings.team_ids_jwt_field is not None:
        ...
        if sso_team_claim_values:
            ...
            verbose_proxy_logger.debug(
                "Loaded teams from DB team_mappings.team_ids_jwt_field='%s': %s",
...
async def _setup_generic_sso_env_vars(
    ...
    generic_discovery_url: Final = os.getenv("GENERIC_DISCOVERY_URL", None)
...
def _build_sso_user_update_data(
    ...
    user_alias: Final = _get_sso_subject(result)
...
class SSOAuthenticationHandler:
    ...
    async def upsert_sso_user(
        ...
        try:
            ...
            retry_user_id: Final = user_defined_values.get("user_id") if user_defined_values is not None else None
...
class MicrosoftSSOHandler:
    ...
    async def get_microsoft_callback_response(
        ...
        subject: Final = MicrosoftSSOHandler.get_subject_from_id_token(id_token=microsoft_sso.id_token)
