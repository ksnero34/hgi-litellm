...
def _sum_counter_units(
    rows: "Iterable[prisma_models.LiteLLM_DailyGuardrailUsageUnits]",
) -> Mapping[str, int]:
...
async def guardrails_usage_detail(
    ...
    flagged: Final = sum(int(m.flagged_count or 0) for m in metrics)
