...
class SyntheticPresidio:
    ...
    async def analyze(self, request):
        ...
        try:
            ...
            text = payload["text"]
            index = text.find("홍길동")
            entities = [] if index < 0 else [{
                "start": index, "end": index + 3, "entity_type": "PERSON", "score": 0.99,
    ...
    async def anonymize(self, request):
        ...
        try:
            ...
            for entity in sorted(payload["analyzer_results"], key=lambda item: item["start"], reverse=True):
                ...
                items.append({"start": entity["start"], "end": entity["start"] + len(replacement),
                              "entity_type": entity["entity_type"], "text": replacement, "operator": "replace"})
...
def cache_for(redis, enabled=True):
    return AnalysisCache(AnalysisCacheConfig(
        enabled=enabled, complete_analysis_verified=True,
        secret="synthetic-secret-for-tests-only-32-bytes", key_version="k1", analysis_version="synthetic-v1",
    ), redis)


def guardrail_for(url, cache, action=PiiAction.MASK):
    return _OPTIONAL_PresidioPIIMasking(
        presidio_analyzer_api_base=url, presidio_anonymizer_api_base=url,
        guardrail_name="synthetic-presidio", output_parse_pii=True,
        pii_entities_config={"PERSON": action}, presidio_language="ko",
        presidio_max_parallel_requests=2, presidio_analysis_cache=cache,
    )
            ...
            for guardrail, expected_status in ((cold, "miss"), (warm, "hit"), (off, "miss")):
                ...
                async with tenant_request():
                    masked = await guardrail.apply_guardrail(
                        {"texts": [text], "text_sources": [{"type": "message", "path": "messages[0].content", "scope": "current_user_prompt"}]},
                        data, "request",
                    )
                assert masked["texts"] == ["🙂\r\n  <PERSON_1>  끝"]
        ...
        try:
            async with tenant_request():
                await asyncio.gather(*(
                    guardrail.apply_guardrail({"texts": [f"synthetic {i}-{j}" for j in range(3)]},
                                              {"metadata": {}}, "request")
                    for i in range(3)
                ))
    ...
    async with local_presidio() as (server, url):
        ...
        try:
            ...
            async with tenant_request() as context:
                ...
                with pytest.raises(_PresidioServiceError):
                    await guardrail.apply_guardrail({"texts": [f"홍길동 {index}" for index in range(8)]},
                                                   {"metadata": {}}, "request")
...
async def test_native_chat_authenticated_team_and_effective_language_separate_cache():
    ...
    async with local_presidio() as (server, url):
        guardrail = _OPTIONAL_PresidioPIIMasking(
            presidio_analyzer_api_base=url, presidio_anonymizer_api_base=url,
            guardrail_name="synthetic-presidio", default_on=True, event_hook="pre_call",
            output_parse_pii=True, presidio_language="ko", presidio_analysis_cache=cache_for(redis),
        )
        try:
            for team, language in (("synthetic-a", "ko"), ("synthetic-a", "ko"),
                                   ("synthetic-b", "ko"), ("synthetic-a", "en")):
                data = {"messages": [{"role": "user", "content": "홍길동"},
...
async def test_responses_instructions_blocks_and_function_arguments_share_context():
    ...
    async with local_presidio() as (server, url):
        guardrail = guardrail_for(url, cache_for(redis))
        try:
            data = {"instructions": "홍길동", "metadata": {"guardrail_config": {"language": "en"}},
                    "input": [{"role": "user", "content": [{"type": "input_text", "text": "홍길동"}]},
                              {"type": "function_call", "call_id": "synthetic-call", "name": "synthetic",
                               "arguments": "홍길동"}]}
            async with tenant_request():
...
async def test_native_configured_one_limits_larger_outer_request_context():
    ...
    async with local_presidio() as (server, url):
        guardrail = _OPTIONAL_PresidioPIIMasking(
            presidio_analyzer_api_base=url, presidio_anonymizer_api_base=url,
            guardrail_name="synthetic-presidio", default_on=True, event_hook="pre_call",
            presidio_max_parallel_requests=1, presidio_analysis_cache=cache_for(SyntheticRedis(), False),
        )
        try:
            with analysis_request_scope(tenant_scope="team:synthetic", max_concurrency=8):
                await guardrail.async_pre_call_hook(UserAPIKeyAuth(team_id="synthetic"), DualCache(),
        ...
        try:
            with analysis_request_scope(tenant_scope="team:synthetic", max_concurrency=8):
                results = await asyncio.gather(*(
                    guardrail.apply_guardrail({"texts": [f"홍길동 {index}-{fragment}" for fragment in range(2)]},
                                              {"metadata": {}}, "request")
