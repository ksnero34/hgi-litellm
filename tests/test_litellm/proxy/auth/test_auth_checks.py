...
async def test_cache_key_object_uses_bounded_ttl_for_near_expiry_key():
    ...
    cache_call = cache.async_set_cache.await_args.kwargs
    cached_value = cache_call["value"]

    assert cache_call["key"] == "active-hash"
    assert cache_call["model_type"] is UserAPIKeyAuth
    assert cache_call["ttl"] == 30.0
    assert cached_value is not key
    assert cached_value.token == "active-hash"
    assert cached_value.expires == now + timedelta(seconds=30)
    assert key.last_refreshed_at is not None
    assert cached_value.last_refreshed_at == key.last_refreshed_at
...
def test_get_experimental_ui_login_jwt_auth_token_valid(valid_sso_user_defined_values):
    ...
    assert token_data["is_session_token"] is True
...
def test_get_key_object_from_ui_hash_key_valid(
    ...
    assert key_object.is_session_token is True
