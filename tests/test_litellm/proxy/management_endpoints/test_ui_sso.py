...
class TestProcessSSOJWTAccessToken:
    ...
    def test_process_sso_jwt_access_token_no_result(self, mock_jwt_handler, sample_jwt_token):
        ...
        with patch("jwt.decode") as mock_jwt_decode:
            result = process_sso_jwt_access_token(
                access_token_str=sample_jwt_token,
                sso_jwt_handler=mock_jwt_handler,
                result=None,
            )
...
@pytest.mark.asyncio
async def test_saml_callback_does_not_enforce_free_sso_user_limit():
    """HGI SAML login remains available when more than five users exist."""
    from litellm.proxy.management_endpoints.ui_sso import saml_callback
    from litellm.proxy.management_endpoints.types import CustomOpenID
    ...
    redirect_response = SimpleNamespace(status_code=302)
    count_billable_users = AsyncMock(return_value=6)
    ...
    with (
        ...
        patch(
            ...
            new=count_billable_users,
        ),
        patch(
            "litellm.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler.get_redirect_response_from_openid",
            new=AsyncMock(return_value=redirect_response),
        ),
    ):
        response = await saml_callback(request_double)

    assert response is redirect_response
    count_billable_users.assert_not_awaited()
