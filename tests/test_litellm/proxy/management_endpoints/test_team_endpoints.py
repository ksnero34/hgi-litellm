...
def test_update_team_metadata_fields_keeps_other_premium_checks():
    ...
    assert updated_kv["metadata"] == {"tags": ["production"]}
    assert "tags" not in updated_kv
...
def test_team_member_add_allows_admin_role_without_premium_license():
    result = team_call_validation_checks(
...
@pytest.mark.asyncio
@pytest.mark.parametrize("action", ["updated", "deleted"])
async def test_team_membership_update_and_delete_emit_distinct_audit_actions(action):
    from litellm.proxy._types import LitellmTableNames
    from litellm.proxy.management_endpoints.team_endpoints import _create_team_membership_audit_log

    with patch(
        "litellm.proxy.management_helpers.audit_logs.create_object_audit_log",
        new_callable=AsyncMock,
    ) as audit:
        result = await _create_team_membership_audit_log(
            team_id="team-1",
            before_members=[Member(user_id="u1", role="user")],
            after_members=[] if action == "deleted" else [Member(user_id="u1", role="admin")],
    ...
    assert audit.call_args.kwargs["table_name"] == LitellmTableNames.TEAM_MEMBERSHIP_TABLE_NAME
