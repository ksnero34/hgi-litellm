...
def test_generic_cost_per_token_bedrock_mantle_gpt56_long_context(_local_model_cost_map, model):
    ...
    assert round(short_completion_cost, 10) == round(model_cost_map["output_cost_per_token"] * completion_tokens, 10)
    ...
    assert round(long_prompt_cost, 10) == round(
        model_cost_map["input_cost_per_token_above_272k_tokens"] * (long_prompt_tokens - cached_tokens)
        + model_cost_map["cache_read_input_token_cost_above_272k_tokens"] * cached_tokens,
        10,
    )
...
def test_generic_cost_per_token_gpt56(
    _local_model_cost_map, model, input_cost, output_cost, cache_read_cost, cache_write_cost
):
...
def test_generic_cost_per_token_gpt56_flex_above_272k(
    _local_model_cost_map, model, flex_long_input_cost, flex_long_output_cost
):
...
def test_generic_cost_per_token_gpt56_terra_cache_costs_by_tier_and_context(
    _local_model_cost_map, service_tier, prompt_tokens, input_rate, cache_write_rate, cache_read_rate
):
...
def test_gemini_36_flash_and_35_flash_lite_launch_pricing(
    _local_model_cost_map, model, input_cost, output_cost, cache_read_cost
):
...
@pytest.mark.parametrize("service_tier,input_rate,output_rate,cache_read_rate", GEMINI_36_FLASH_SERVICE_TIER_PRICING)
@pytest.mark.parametrize("model", ["gemini-3.6-flash", "gemini/gemini-3.6-flash", "vertex_ai/gemini-3.6-flash"])
...
def test_generic_cost_per_token_grok_46_long_context(_local_model_cost_map):
    usage = Usage(
        ...
        completion_tokens=1_000,
        total_tokens=251_000,
        prompt_tokens_details=PromptTokensDetailsWrapper(cached_tokens=50_000, text_tokens=200_000),
    )
    prompt_cost, completion_cost = generic_cost_per_token(
        model="grok-4.6",
        usage=usage,
